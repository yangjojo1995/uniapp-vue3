      ...
      >
        <template #content>
          <view class="field-container">
            <view class="field">
              <view class="field__label">报名姓名</view>
...
const filterData = reactive({
  activityId: '1526499318529064962'
})

// 分页
const listPagingLazy = reactive(
  new PagingLazy(async function (page, size) {
    const { total, records } = await service.activity.activityApply({
      page,
      size,
      ...filterData
    })
...
const handleClick = function (item: any) {
  ...
  router.push({ path: '/pages/activity/apply/detail', query: { activityId: item.id } })
}
</script>
