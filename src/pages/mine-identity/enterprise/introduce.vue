	...
	<view class="main-container">
			<view >
				<view class="login-form-item u-p-y-20">
					<textarea v-model="params.summary"  class="login-textara" :maxlength="-1"  placeholder-class="login-placeholder" placeholder="请输入公司简介(2000字以内)"  />
				</view>
			</view>
...
const submit = async () => {
  ...
  isSubmit = true
	...
	try {
		if (!isEdit) {
			const result = await service.certification.addEnterpriseDraft(params)
			setTimeout(() => {
				uni.hideLoading()
				isSubmit = false
				uni.navigateBack({
			...
			uni.$emit('enterprise:summary', params.summary)
			uni.navigateBack({
				delta:1 
			})
		}
