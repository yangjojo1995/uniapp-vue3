    ...
    <view class="u-p-x-32 u-p-t-20">
      <view class="bg-white u-m-b-20 u-p-x-30 u-p-y-40 flex jc-between ai-center" v-for="item,index in listPagingLazy.list" :key="index" @click="selectChange(index,item)">
        <view class="avatar-icon u-m-r-20" >
            <image :src="assets(item.logo) || '../../../assets/mine/default.png'"/>
        </view>
        <view class="flex-1 ai-strech flex flex-column jc-between u-m-r-30" >
          <view class="fs-30 text-color-primary u-m-b-10 bold">{{item.fullName}}</view>
        </view>
        <view class="checkbox">
...
const goSubmit = async () => {
  ...
  if (!selectBoolean) {
    ...
    uni.showLoading({})
    try {
      const result = await service.manager.applyRelation({investInstitutionId:investInstitutionId})
      setTimeout(() => {
        uni.hideLoading()
        uni.navigateTo({url:'/pages/mine-identity/account-manage/state-relative'})
      }, 500)
...
const listPagingLazy = reactive(new PagingLazy(async function (page, size) {
  const { total, records }  = await service.organization.getOrganizationRecords({ page, size, name: searchValue.value, ...filterData  })
