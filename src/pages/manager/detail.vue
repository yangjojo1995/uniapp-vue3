  ...
  <view class="page">
    ...
    <view class="header" :class="{'enable-state': data.enable === 1}">
      ...
      <view class="header__icon">
        <image class="header-icon" mode="aspectFit" :src="assets(data.avatar)" alt="" />
      </view>
    ...
    <ym-group title="基本信息" type="info">
      <view class="field">
        ...
        <view class="field__value">{{ data.investInstitution }}</view>
      </view>
      <view class="field">
        <view class="field__label">职位</view>
        <view class="field__value">{{ data.position || '-' }}</view>
      </view>
      <view class="field">
        <view class="field__label">联系电话</view>
        <view class="field__value">{{ data.phone || '不公开' }}</view>
      </view>
      <view class="field">
        <view class="field__label">邮箱地址</view>
        <view class="field__value">{{ data.email || '不公开' }}</view>
      </view>
      <view class="field">
        <view class="field__label">所属地区</view>
        <view class="field__value">{{ data.belongProvinceStr }} {{ data.belongCityStr }} {{ data.belongDistrictStr }}</view>
      </view>
    </ym-group>
    <ym-group title="个人简介" type="info">
      <view class="field">
        <view class="field__value2 ym-text-pre-wrap">{{ data.summary }}</view>
      </view>
    </ym-group>
  </block>
  </view>
...
onLoad(async (props) => {
  ...
  const result = await service.manager.getManagerInfo({ id })
...
const goCredibility = () => {
  uni.navigateTo({	url: `/pages/mine/credibility?logo=${data.avatar}&name=${data.name}` })
}

const sayHello = async () => {
  console.log(await getIdentity())
