    ...
    <view class="form">
      ...
      <view class="form-group">
        ...
        <view class="form-group-description" v-if="showMeg">
          <view v-if="userInfo.email"
            >已发送至邮箱{{ userInfo.email }}</view
          >
        </view>
...
<script setup lang="ts">
  ...
  // 用户信息，用于显示验证码提示信息
  let userInfo: userInterface = reactive({ id: '', userName: '', email: '' })
  onLoad(async (props) => {
  ...
  const handleSendCode = async (): Promise<void> => {
    showMeg.value = true
    if(typeof( userInfo.email) !== 'string') return
    let res: boolean = await service.app.sendCode({account: userInfo.email, template: 'bind-email' })    
  ...
  const codeFinish = async (e: number) => {
    ...
    isSubmit.value = true
    try {
      const result = await service.mine.checkEmailCode({ code: e })
      uni.navigateTo({
