...
import { ref, onLoad, reactive, useService, useConfig, requireCheck, operateConfirm, onShow} from '@/utils/commonUse'
...
const goBase = () => {
  uni.navigateTo({url: './form-base?id=' + params.id + '&type='+type.value})

}

const goIntroduce = () => {
  uni.navigateTo({url: './form-introduce?id=' + params.id +'&type='+type.value})
}

// 提交表单
const confirmForm = () => {
  if(requireCheck('请补全活动详情', params.content)) return
  operateConfirm('发布', async () => {
    return await service.activity.commitDraft({id: params.id})
  }, function () {
