<template>
  <view>
    <ym-group type="info" v-if="data.title">
      <template #title>
        <!-- 绑定类型0挂靠机构邀请（投资经理）1挂靠机构申请（投资机构）2子账号邀请（个人：普通用户和投资经理）	 -->
      ...
      <template v-if="data.bindType == 3">
        ...
        <view class="field" >
          ...
          <view class="field__value">{{ data.targetName || '-' }}</view>
        </view>
      </template>
      <template v-else-if="data.bindType == 4">
        <view class="field" >
          <view class="field__label">项目名称：</view>
          <view class="field__value">{{ data.projectName || '-' }}</view>
        </view>
    ...
    <view class="f-btn" v-if="data.status == '0'">
      <view class="flex-center green-bg footer-btn" @click="submits">撤销</view>
...
const submits = async () => {
  await service.review.reviewOperate({ id, operate: 'undo' })
  uni.showToast({
    icon: 'success',
    title: '操作成功'
  })
...
::v-deep.group-header__inner{
  ...
  .btn-error {
    color: #6b54d4;
    background: rgba(107, 84, 212, 0.1);
    border: 2rpx solid rgba(107, 84, 212, 0.3);
  }
  .text-primary {
    color: #00c682;
  }
